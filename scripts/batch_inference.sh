#!/usr/bin/env bash
#
# Batch inference script for CodeTalker.
...
Options:
  -o, --output-dir DIR      Output directory (default: from config demo_output_path)
  ...
  --skip-existing           Skip audios whose output videos already exist
...
if [[ -z "$OUTPUT_DIR" ]]; then
    OUTPUT_DIR="$(_read_cfg demo_output_path)"
    if [[ -z "$OUTPUT_DIR" ]]; then
...
for idx in "${!AUDIO_FILES[@]}"; do
    ...
    # Output naming follows CodeTalker convention: {name}_{subject}_condition_{condition}_audio.mp4
    out_name="${audio_name}_${SUBJECT}_condition_${CONDITION}_audio.mp4"
    ...
    # Build the command — override wav path, condition, subject, output via config opts
    CMD=(
        .venv/bin/python main/demo.py
        --config "$CONFIG"
    ...
    if [[ -n "$OUTPUT_DIR" ]]; then
        CMD+=(demo_output_path "$OUTPUT_DIR")
    fi
    ...
    if "${CMD[@]}" 2>&1; then
        ...
        echo "  -> Done: $out_name ($(fmt_duration $FILE_ELAPSED))"
    else
        FILE_END=$(date +%s)
        FILE_ELAPSED=$((FILE_END - FILE_START))
        TOTAL_PROCESSING_TIME=$((TOTAL_PROCESSING_TIME + FILE_ELAPSED))
